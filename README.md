...
*   **最小権限の原則**: GitHub Actions 用の IAM ロール（OIDC認証）は、対応する環境（`develop` ブランチは `dev` ロール、`main` ブランチは `prod` ロールのみを引き受け可能）へ厳密に分離し、過剰な権限付与を防いでいます。また、GitHub Secrets の管理負荷を極小化するため、ワークフロー内で OIDC ロール ARN から対象の AWS アカウントID (12桁) を動的に自動抽出する動的ルーティング設計を導入しています。
...
*   **開発コストの極小化とスケジュール制御**: 開発環境（`dev`）ではオートスケーリングを排し、プラットフォーム共通の Resource Scheduler によるデータベースの夜間一時停止を適用。クラウド費用を節約します。
...
*   **夜間自動停止の適用**: プラットフォーム共通の Resource Scheduler（タグ `Schedule = office-hours` 連携）を用い、夜間（20:00〜翌朝8:00）に Aurora Serverless v2 を自動一時停止させ、稼働時間を削減。
